...
def _make_sample_factory_integration(grid_config, goal_sequences, wait_steps):
    env = _make_pogema(grid_config, goal_sequences, wait_steps)
    env = MetricsForwardingWrapper(env)
    env = IsMultiAgentWrapper(env)
    if grid_config.auto_reset is None or grid_config.auto_reset:
        env = AutoResetWrapper(env)
...
def make_pogema(grid_config: Union[GridConfig, dict] = GridConfig(), *args, goal_sequences=None, wait_steps=None, **kwargs):
    ...
    if grid_config.integration is None:
        return _make_pogema(grid_config, goal_sequences=goal_sequences, wait_steps=wait_steps)
    elif grid_config.integration == 'SampleFactory':
        return _make_sample_factory_integration(grid_config, goal_sequences=goal_sequences, wait_steps=wait_steps)
