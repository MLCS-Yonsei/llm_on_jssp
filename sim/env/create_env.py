...
def create_env_base(config: Environment,goal_sequences=None, wait_sequences=None,agents_start_pos=None ):
    
    if wait_sequences is not None:
        config.grid_config.agents_xy =  agents_start_pos    
        config.grid_config.targets_xy = [(0, 7)] * len(config.grid_config.agents_xy)

    env = pogema_v0(grid_config=config.grid_config,goal_sequences= goal_sequences,wait_steps=wait_sequences)
